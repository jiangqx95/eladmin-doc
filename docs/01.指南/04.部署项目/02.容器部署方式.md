...
![imageb817a495226b07de.jpg](/gkadmin-doc/images/2020/09/17/imageb817a495226b07de.jpg)
...
[具体参考](https://github.com/elunez/gkadmin/commit/029a932ff96e282f316809af30a176764eafde43)
:::

修改完成后，打包项目将打包好地Jar上传到服务器的 `/home/gkadmin` 目录，在该目录创建 Dockerfile 文件，并编写 Dockerfile
...
构建镜像
```
docker build -t gkadmin .
```
#### 启动Java容器
将容器中的 `/home/gkadmin/` 挂载到宿主机的 `/home/data/` 目录并且设置数据库地址与密码等环境变量参数

```
docker run -d \
--name gkadmin --restart always \
-p 8000:8000 \
-e "TZ=Asia/Shanghai" \
-e DB_HOST=172.17.0.1 \
-e DB_PWD=mysql_pwd \
-e REDIS_HOST=172.17.0.1 \
-v /home/data/:/home/gkadmin/ \
gkadmin
```
...
在 `/home/nginx/conf.d` 创建一个 gkadmin.conf 的配置文件，文件内容如下
...
server
    {
        ...
        server_name 域名/外网IP;
        index index.html;
        root  /usr/share/nginx/html/gkadmin/dist;  #dist上传的路径
...
```
注意：容器中 `/usr/share/nginx/html/gkadmin/dist` 对应宿主机的 `/home/nginx/html/gkadmin/dist` 目录，因此文件上传到宿主机的目录即可
...
![imageb0806c848d01318e.jpg](/gkadmin-doc/images/2020/09/17/imageb0806c848d01318e.jpg)

2、修改 `api.js` 中的 `baseUrl = process.env.VUE_APP_BASE_API === '/' ? '' : process.env.VUE_APP_BASE_API`

![imagedcd151a815c62932.jpg](/gkadmin-doc/images/2020/09/17/imagedcd151a815c62932.jpg)

3、打包项目并且上传到 `/home/nginx/html/gkadmin` 目录下
